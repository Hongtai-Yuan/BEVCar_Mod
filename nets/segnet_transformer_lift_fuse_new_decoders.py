...
class SegnetTransformerLiftFuse(nn.Module):
    def __init__(self, Z_cam: int, Y_cam: int, X_cam: int, Z_rad: int, Y_rad: int, X_rad: int,
        ...
        from easydict import EasyDict as edict   # 方便 dot 访问
        
        # class_names = ['car','truck','bus','trailer','construction',
        #        'pedestrian','motorcycle','bicycle','traffic_cone','barrier']
        # 'Car', 'Van', 'Truck', 'Pedestrian',  'Person_sitting', 'Cyclist', 'Tram', 'Misc' or 'DonCare'
        class_names = ['Car','Cyclist','Pedestrian','Truck','Motorcycle',
               'Dense_bicycle','Bus','Tricycle','Traffic_signs','Construction']
        dx = dy = dz = 0.2
        point_cloud_range = [-61.2, -61.2, -10.0,  61.2, 61.2, 10.0]
        grid_size = np.array([
            int((point_cloud_range[4]-point_cloud_range[1]) / dy),  # X 轴
            int((point_cloud_range[3]-point_cloud_range[0]) / dx),  # Y 轴
            1                                                     # Z 轴
        ]) 
        ...
        # Decoder
        self.det_head = TransFusionHead(
            model_cfg=model_cfg_tf,
            input_channels=latent_dim,          # 128
    ...
    def forward(self, rgb_camXs: torch.Tensor, pix_T_cams: torch.Tensor, cam0_T_camXs: torch.Tensor,
                vox_util: torch.Tensor, rad_occ_mem0: torch.Tensor = None, gt_boxes: torch.Tensor | None = None) -> torch.Tensor:
        """
        Args:
        ...
        # bev decoder
                # ---------- bev Enc-Dec ----------
        feat_bev = self.feat_enc_dec(feat_bev_) if self.do_feat_enc_dec else feat_bev_

        # ---------- 1. 先 stride=8 下采样 (200→25) ----------
        feat_bev_det = F.avg_pool2d(feat_bev, kernel_size=8)          # (B,128,25,25)

        # ---------- 2. 再插值到 76×76，使其与 TransFusionHead 的嵌入长度一致 ----------
        feat_bev_det = F.interpolate(
            feat_bev_det, size=(76, 76), mode='bilinear', align_corners=False
        )                                                             # (B,128,76,76)

        # ---------- 2. 构造 batch_dict，送入检测头 ----------
        batch_dict = {'spatial_features_2d': feat_bev_det}
        if self.training:
            assert gt_boxes is not None, "training=True 时必须给 gt_boxes"
            batch_dict['gt_boxes'] = gt_boxes      # :contentReference[oaicite:0]{index=0}

        batch_dict = self.det_head(batch_dict)  # 前向

        # ---------- 3. 根据模式返回 ----------
        if not self.training:                   # 推理
            return batch_dict['final_box_dicts']  # list[dict] 每 dict 含 boxes/scores/labels

        # 训练：只回传检测 loss 与日志
        loss_det  = batch_dict['loss']
        tb_dict_det = batch_dict['tb_dict']
        return loss_det, tb_dict_det
