...
class FusionDataset(DatasetTemplate):
    ...
    def get_arbe(self, idx):
        ...
        points_arbe_unified = np.concatenate([points_arbe, np.zeros((num_point, 1))], axis=-1)

        return points_arbe_unified
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if count_inside_pts:
                    ...
                    calib = self.get_calib(sample_idx)
                    pts_rect_lidar = calib.lidar_to_rect(points_lidar[:, 0:3])
                    pts_rect_arbe = calib.lidar_to_rect(points_arbe[:, 0:3])
                    fov_flag = self.get_fov_flag(pts_rect_lidar, info['image']['image_shape'], calib)
                    #这里是取相机视角范围内的点
                    pts_fov = points_lidar[fov_flag]
                    #返回激光坐标系下的3D框
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            for i in range(num_obj):
                ...
                #通过索引获取到相应3dbox内的点
                gt_points = points_lidar[point_indices[i] > 0]
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            ...
            if self.dataset_cfg.FOV_POINTS_ONLY:
                pts_rect = calib.lidar_to_rect(points_lidar[:, 0:3])
                fov_flag = self.get_fov_flag(pts_rect, img_shape, calib)
                points_lidar = points_lidar[fov_flag]
            input_dict['points_lidar'] = points_lidar
